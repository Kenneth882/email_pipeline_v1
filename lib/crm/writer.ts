...
export function buildReviewReason(triage: TriageResult): string {
...
export async function runCrmWriter(opts: {
  ...
  const reviewReason = buildReviewReason(triage);
  const targetStage = resolveTargetStage(triage, icpVerdict);
  ...
  try {
    await updateContactProperties(venue.hubspot_contact_id, {
      ...
      needs_review: triage.needs_human_review,
      review_reason: reviewReason || null,
      key_details: (triage.extracted.key_details ?? []).join("\n") || null,
      thread_id: threadId,
